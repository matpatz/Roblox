...
export default async function handler_fn(req, res) {
  ...
  const { id } = req.query;
  if (!id) throw new ApiError(400, 'Missing script id');
  ...
  if (req.method === 'GET') {
    ...
    if (error || !data) throw new ApiError(404, 'Script not found');
  ...
  if (req.method === 'PUT') {
    ...
    if (error) throw new ApiError(500, 'Failed to update script');
    if (!data) throw new ApiError(404, 'Script not found');
    return successResponse(res, req, data);
  }
  ...
  if (req.method === 'DELETE') {
    ...
    if (error) throw new ApiError(500, 'Failed to delete script');
