-- https://sneekysscripts.uk/Scripts/UIs/silent_aim.luau
...
return function(sFOV: number, tFunc: (origin: Vector3?) -> (Model | BasePart)?, mCenter: boolean?)
    ...
    -- The draw lib calls the target function once per frame; cache the result and only re-scan at a fixed rate.
    local ScanInterval: number = 0.05;
    local LastScan: number = -math.huge;
    local CachedTarget: (Model | BasePart)?;

    local TargetFunction = function(OriginPosition: Vector3?): (Model | BasePart)?
        local Now = os.clock();
        if Now - LastScan >= ScanInterval then
            LastScan = Now;
            CachedTarget = tFunc(OriginPosition);
        end;
        return CachedTarget;
    end;

    local draw = Drawlib.new(getgenv().sneeky_fov_size or sFOV, TargetFunction, mCenter);

    local p = Instance.new("ScreenGui", hui);
    p.Name = code(p:GetDebugId());

    local menu = Instance.new("Frame", p)
    ...
    menuCorner.Name = code(menuCorner:GetDebugId());
